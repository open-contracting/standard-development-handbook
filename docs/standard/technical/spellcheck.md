# Spell-checking

## Operation

If you have `aspell` installed, run:

```shell
find . -type f -not -path '*/\.*' -not -path '*/_static/*' -not -path '*/codelists_translated/*' -not -name 'Makefile' -not -name '*.bat' -not -name '*.css' -not -name '*.html' -not -name '*.js' -not -name '*.mo' -not -name '*.png' -not -name '*.po' -not -name '*.py' -not -name '*.pyc' -not -name '*.scss' -not -name '*.sh' -not -name '*.svg' -not -name '*.txt' -exec aspell -H check '{}' ';'
```

This command will skip dot files, Docson files, translated codelists, and css, html, js, mo, png, po, py, pyc, sh, svg and txt files.

## Configuration

`aspell` will flag many field names and proper nouns as errors. `aspell` allows you to add words to its dictionary during operation. Instead of re-adding the following words, simply replace `~/.aspell.en.pws` this the following:

```text
personal_ws-1.1 en 672 
foreignBidsFromEU
evaluationCriteria
Extractives
positiveInteger
maxLength
Popolo
versionedRecord
hearingNotice
relatedTransactions
MultiLineString
sublicensees
riskImpact
mezzanineDebt
netPresentValueDetails
procurementMethod
versioning
eval
Addler
developmentStarted
enquiryResponses
tenderers
Dar
postalCode
netPresentValueRateDetails
requirementGroups
jsonschema
financeSummary
streetAddress
relatedTenderer
awardPeriod
OrganizationReference
DGRMSG
SMEs
procurements
disaggregated
maxProperties
additionalProperties
Marchessault
OrganizationUnversioned
ocds
dev
accessDetails
IANA
weightage
OCHA
partiallyMet
UNSPSC
participationFee
Habibullah
compiledReleases
BSI
terminologist
nonfinancial
ocid
awardStatus
datePublished
CPC
COH
Rozo
relatedImplementationMilestone
dataType
charset
dir
AwardUnversioned
timeline
paidBy
dateMet
geometryType
modality
idRef
Oxfordshire
shortlistedFirms
lowestCost
revenueShare
TenderUnversioned
OECD
NGOs
CPV
threadID
Transifex
deliveryLocation
Serghi
conflictOfInterest
camelCase
IATI
Onerhime
qualifiedBidders
relatedBid
availableLanguage
accessURL
csv
boolean
pageEnd
technicalSpecifications
developmentComplete
codelists
guaranteeReports
debtEquityRatioDetails
GBP
privateParty
versioned
Robichaud
Kaplan
contractSummary
czf
subsidyRatioDetails
procurementMethodRationale
Carlier
ExchangeRate
estimatedDemand
majeure
costOnly
virtualenv
tariffMethod
validator
LineString
preferredBidders
operationPerformance
jsoninclude
Sisti
GGP
Schouten
versionId
evaluationCommittee
multipleOf
minValue
countryName
awardCriteriaDetails
packageMetadata
shortlistUpdate
valueForMoneyAnalysis
helpdesk
CCSA
financeArrangements
timeOfDay
Durations
EU's
subsidyRatio
contractAnnexe
unitOfMeasure
submissionMethod
physicalProgressReport
Mihály
pageStart
relatedProcess
Dewhurst
awardTwo
README
relatedRequirementID
schemas
sourceEntity
shareCapital
physicalProcessReport
landAvailability
Bohórquez
enquiries
oneOf
relatedItem
financeCategory
deprecatedVersion
APIs
discountRate
financialClose
replacementProcess
templating
namespaces
Meike
eligibilityCriteria
exclusiveMaximum
sublicense
APMG
linkable
consortiaMember
PerformanceFailure
maxValue
performanceReport
releaseTag
Montiel
estimatedValue
Nyager
renewalProcess
Redpath
procurementMethodDetails
consultingServices
durationInDays
Gruden
partyDetail
Secretaría
awardNotice
enquiryPeriodEnd
Edafe
tariffIllustration
PeriodUnversioned
GeoJSON
Paetzold
IRR
projectDetail
GSIN
RelatedProcessUnversioned
contractSigned
datasets
votingRights
bidStatistics
environmentalImpact
Mathieu
counterparty
deliverables
contractUpdate
OKDP
publicationPolicy
AmendmentUnversioned
disqualifiedBidder
metadata
OLAP
methodOfPayment
organizationIdentifierRegistrationAgency
openCodelist
Leduc
Bista
KPI
amendsReleaseID
CKAN
OrganizationReference's
BidsStatistic
mergeStrategy
UNCEFACT
Greentree
availabilityAndQuality
assetAndLiabilityAssessment
renegotiations
organizationRole
releaseID
uniqueItems
enquiry
finalAudit
otherRevenue
reviewBody
positiveIntegerDefault
Whitehouse
featureOf
Berners
marketStudies
relatedMilestone
bestProposal
prepend
inflationAndFx
Dudis
Licensor
OKPD
submissionDocuments
totalValue
Enaholo
initiationType
backend
Muqbel
REITs
pppModeRationale
subtype
Kami
Neontribe
otherGovernmentSupport
codelist
preProcurement
OCP
enquiryPeriodStart
programme
penaltyContracted
requestForQualification
ImplementationUnversioned
SimpleHTTPServer
AnyCounty
preferredBidder
Aldate's
maxdepth
tenderUpdate
MilestoneUnversioned
contractClose
wholeListMerge
awardCriteria
annualDemand
nplurals
namespace
stringArray
ReadTheDocs
pdf
performanceFailures
advisors
Noé
foreignBids
notMet
milestoneCode
versionedRelease
changeInLaw
minItems
publishedDate
grantor
PlanningUnversioned
BudgetBreakdownUnversioned
PFI
avgToll
SquareMile
msgid
Raad
consultancyServices
simpleTypes
maximumLotsAwardedPerSupplier
sourceParty
beneficialOwners
hasEnquiries
billOfQuantity
equityTransferCaps
Crédito
OpenStreetMap
geolocated
forceMajeure
vehicleType
assetTransfer
MERCHANTABILITY
approvalLetter
Alonso
feasibilityStudy
relatedProcessScheme
sectoral
forma
procurementPlan
participationFees
startDate
RDF
png
Domínguez
neighboring
contactPoint
NONINFRINGEMENT
arrayMergeById
budgetBreakdown
ShareholderUnversioned
Heydon
licensable
qualificationAmendment
Hercher
additionalClassifications
relatedLot
anyOf
lotValues
awardCancellation
ocdsmerge
Fazekas
agreedMetrics
ContractUnversioned
pre
ppp
RFP
DocumentUnversioned
netPresentValue
RFQ
socialWitness
Brandusescu
OCIDs
maxItems
additionalItems
Jiri
ratedCriteria
ocdsVersion
html
funder
patternProperties
chargePaidBy
winningBid
implementers
lotDetails
equityInvestor
config
Martínez
optionToCombine
projectAdditionality
extensiontable
nocrossref
Público
landTransfer
genindex
mergeOptions
yyyy
repaymentFrequency
projectID
commercialClose
txconfig
dateModified
LotGroup
providerOrganization
tenderAmendment
URIs
riskProvisions
contractPeriod
draftFinalTender
serviceDescription
responseSource
extensionlist
MultiPolygon
tenderCancellation
minLength
unsuccessfulProcess
Acuna
qualityOnly
omitWhenMerged
qualificationCancellation
timelines
sme
CCCEV
dateAnswered
compiledRelease
qualificationUpdate
OrganizationReferenceUnversioned
constructionStarted
RequirementUnversioned
preQualificationStatus
locationGazetteers
anonymize
contractStatus
TransactionUnversioned
SMS
milestoneType
IdentifierUnversioned
changelog
rst
ItemUnversioned
additionality
partyRole
StringNullDateTimeVersioned
leadBank
Omidyar
uid
tendersAbnormallyLow
Opyr
RequirementReference
SPV
receiverOrganization
Drawdowns
CAPTCHA
additionalContactPoints
exclusiveMinimum
Skuhrovec
dataset
enquiryPeriod
Hamel
OSMN
shareCapitalDetails
Anytown
AnyTown
OSMR
Taggart
svg
actualValue
biddingDocuments
compiledRecord
OSMW
maxExtentDate
jsonpointer
CPVS
Myroslav
evaluationReports
tenderPeriod
contractGuarantees
maximumValue
modindex
PPPIRC
Halton
riskComparison
interestRate
debarments
contractArrangements
inPerson
relatedLots
interestedParty
certificationLevel
additionalProcurementCategories
FCNY
Begtin
cancellationDetails
riskLikelihood
Irum
RiskUnversioned
COFOG
geocoding
contractDraft
publicBondIssue
requirementResponses
uri
USD
schemaArray
url
accessLevel
UTC
tenderStatus
preQualification
WGS
aggregators
UTF
MilestoneReference
dueDate
Inkscape
usr
txt
financeAdditionality
riskPremiumDetails
extensionselectortable
minProperties
PPPs
searchable
electronicSubmission
Raznick
Szoke
singleBidOnly
MilestoneReferenceUnversioned
votingRightsDetails
bidStatus
submissionMethodDetails
stepInRights
Mandelbaum
partyScale
legalName
centric
milestoneStatus
endDate
debtEquityRatio
allOf
interoperability
penaltyImposed
otherWitness
ClassificationUnversioned
SDGs
Marcela
ContactPointUnversioned
Redactions
dateSigned
cd
beneficialOwnership
RequirementGroup
PyCon
financeType
funders
Krzysiek
financialIndicator
disqualifiedBidders
ef
tariffReview
msgstr
awardUpdate
MultiPoint
needsAssessment
assetProvider
Madejski
contractSchedule
designAndConstruction
completionCertificate
Mihai
contractNotice
relatesTo
incrementing
smeBids
procuringEntity
financeAvailability
xargs
financialStatement
StringNullUriVersioned
qualificationPeriod
foundational
json
md
Lorien
toctree
exchangeRateGuarantee
AnyCorp
RequirementGroupUnversioned
numberOfTenderers
GeoNames
publicAuthority
itemClassificationScheme
transactional
Mireille
mainProcurementCategory
contractAmendment
fswatch
maximumLotsBidPerSupplier
Ontologies
implementationUpdate
Bibhusan
supplierCredit
KPIs
Makgill
Seember
fiscalImpact
StringNullVersioned
sd
documentationUrl
electronicAuction
expectedValue
py
defaultEvents
Eurostat
projectPlan
contractTermination
Postelnicu
validBids
enum
projectIRRDetails
procurementCategory
ve
tenderNotice
LIBOR
discountRateDetails
Maqsood
riskCategory
electronicBids
Lenczner
bankability
exchangeRates
extendsContractID
planningUpdate
RequirementResponse
financialProgressReport
subContract
participationFeeType
releaseDate
servicePayments
awardID
unitClassificationScheme
Za
CEFACT
earlyTermination
lotGroups
interoperable
extendedProcurementCategory
constructionComplete
abatements
penaltyPaid
wy
timezones
Validators
evaluationIndicators
EPSG
deliveryAddress
documentType
Maudry
BudgetUnversioned
Oakleigh
CriterionUnversioned
metricID
projectApproval
ValueUnversioned
QUDT
riskAllocation
financingParty
relatedProcesses
BCP
faxNumber
Vozárová
AddressUnversioned
riskPremium
negotiationParameters
projectIRR
prepended
dereferenceable
bestValueToGovernment
additionalIdentifiers
Munilla
Jachym
priceOnly
qualifiedBidder
Barnet
seniorDebt
exchangeRateSource
```
